...
**RESULT (2026-06-06, executed at the 1.5.3 cutover — installed plugin 1.5.3 = `ed640b6`, cache byte-verified and registry-confirmed active; parent session `2d853383…`, throwaway team `spike9`; figures cross-checked with a helper mirroring `latest_main_thread_usage()`):** Facets (1)–(4) ✅ — **Spike 8 facet (3)'s scoping defect is CLOSED.** Facet (5) split: loop guard ✅ / cross-wake once-per-threshold ❌ — one NEW follow-up (per-wake agent-identity churn).

- **(1) ✅ Blocked on its OWN figure.** Teammate `bloater` ended its bloat turn at 254,066 (it flew blind past the 210k target — see (5)'s churn intel — so the single block correctly announced the highest crossed tier per `highest_crossing`): forced-turn report verbatim `[254,066 tokens used] Context checkpoint 250k crossed (turn-end detection)…`. Provenance: 254,066 sits at lines 130–131 of its own wake transcript (the bloat turn's final API call; the forced turn's entries 254,152→255,239 follow) and appears nowhere in the parent transcript (manager at 138,535 at that moment). Bonus: the same agent file under the old sidechain filter computes `latest_total: null` (5/5 entries skipped) — the Spike 8 facet-3 blindness reproduced live, now bypassed by `include_sidechain`.
- **(2) ✅ Parent pools untouched; manager still announces — the 1.5.1 collision is gone.** Parent state was `{"prompt": 0, "tool": 100000, "stop": 0, "subagent_stop": 0}` before the teammate's crossing and byte-identical after BOTH teammate blocks; the writes landed per-agent (`…--a70e7bd0d80070f33.json`, then `…--a2e0f476ef4f955b9.json`, each `{"subagent_stop": 250000}`). The manager's own crossings then fired on both paths: mid-turn `[202,686 tokens used] … 200k crossed` (PostToolUse/`tool`) and turn-end block `[213,023 tokens used] … (turn-end detection)` (Stop/`stop`), leaving `{"prompt": 0, "tool": 200000, "stop": 200000, "subagent_stop": 0}` — `prompt: 0` is the F22 starvation signature again (no keystroke past 100k; inbox wakes fire no UserPromptSubmit).
- **(3) ✅ A manager crossing does not block teammates.** With the manager ≥213k, fresh teammate `quiet`'s turn-end (own transcript 42,093) stayed silent — no block report, no composite state file, parent state unchanged. Under 1.5.1 this exact configuration was the spurious-block case.
- **(4) ✅ No spurious one-shot block.** A one-shot `Explore` subagent completing on the ≥213k session returned `NO-HOOK-FEEDBACK` (it was instructed to echo any checkpoint text verbatim); no state write appeared.
- **(5) ⚠️ Guard ✅ / cross-wake dedup ❌.** `stop_hook_active` held everywhere: no teammate forced turn re-blocked, and the manager's three subsequent sub-250k turn-ends stayed silent (manager state keys on the stable session_id). But **the harness gives every teammate wake a fresh `agent_id` AND a fresh wake transcript** (one teammate, three wakes: `agent-a6b95…` READY → `agent-a70e7…` BLOAT → `agent-a2e0f…` PING), so per-agent once-per-threshold state starts empty each wake: the PING wake — the same teammate, parked at ~255k — was re-blocked verbatim `[255,895 tokens used] Context checkpoint 250k crossed …` and wrote a second composite state file. **Operational meaning: a teammate that stays above an actionable threshold is re-blocked (one forced turn) at EVERY subsequent wake until it hands over or compacts.** The hook's logic is correct as coded; what reality falsified is the design's assumption that `agent_id` is wake-stable. (Churn intel for any fix: the current wake's transcript filename is unknowable before the wake starts — bloater's in-turn self-measurement failed because the path it was given belonged to the previous wake; a self-measuring teammate must glob the newest `subagents/agent-*.jsonl` at wake start.)

**Disposition: facet-(5) follow-up = re-block-per-wake.** Candidate directions, deliberately left to a design pass: (a) accept-as-residual + document — the re-block is a bounded, correct-figure nag toward a handover that is already mandatory (one forced turn per wake; no intra-wake loop); (b) find a wake-stable key in the live SubagentStop payload (needs payload introspection via a scratch logging hook — does a stable teammate name/id field exist?); (c) infer stability another way (e.g. transcript lineage). Until decided, the shipped behavior is: teammate self-detection works, scoped and figure-accurate, with a per-wake repeat while parked over-threshold.

**DECIDED 2026-06-06 (with the 1.5.4 de-engineering): (a) accept-as-residual.** The user's design correction reframed the hook as a pure sensor — warning messages at thresholds; the agent's own operating instructions direct the handling. Under sensor semantics the per-wake repeat is correct delivery, not a defect: *an agent that wakes still over an actionable threshold gets one fresh warning that wake* (bounded: one forced turn per wake, no intra-wake loop, figure always accurate). Directions (b)/(c) are killed as over-engineering. Recorded in the hook docstring as the second accepted residual; shipped in 1.5.4 alongside the neutral sensor rewording of every checkpoint message.
...
6. **(1.5.2 teammate scoping)** After the 1.5.2 release: `claude plugin install claude-toolkit` (restart so the updated hook loads), run **Spike 9**, and record its RESULT inline. Spike 8 facet (3)'s scoping defect is not closed until Spike 9 is GREEN. — **DONE 2026-06-06 against installed 1.5.3 (`ed640b6`): facets (1)–(4) GREEN — Spike 8 facet (3)'s scoping defect is CLOSED. Facet (5) split: guard GREEN / cross-wake once-per-threshold RED — follow-up (re-block at every wake while a teammate stays over-threshold; per-wake agent-identity churn) **dispositioned 2026-06-06: accept-as-residual (1.5.4)**. See Spike 9 RESULT.**
...
**Spike 8 (turn-end threshold detection) EXECUTED 2026-06-06 at the 1.5.1 cutover — facets (1)/(2)/(4) ✅ GREEN; facet (3) delivery ✅ / scoping ❌** (see its RESULT block). Theme A's manager-side detection is **done**. Exactly one open follow-up remains, inherited from facet (3): **teammate-scoped SubagentStop measurement** — teammate turn-end events carry the parent's session_id, and teammate transcripts (`<parent-session>/subagents/agent-<id>.jsonl`, all entries `isSidechain: true`) are invisible to the hook's sidechain filter, so teammates are currently blocked on the *manager's* usage, not their own. Fix shape: measure the agent transcript named by the payload without the sidechain filter and key state per agent — a hook-only change — **designed 2026-06-06** (`docs/superpowers/specs/2026-06-06-teammate-scoped-context-checkpoints-design.md`), implemented as **1.5.2** on branch `teammate-scoped-checkpoints`; **verified by Spike 9 at the 1.5.3 cutover (2026-06-06): facets (1)–(4) GREEN — the scoping defect is CLOSED.** Spike 9's facet (5) replaced it with one final follow-up: **per-wake agent-identity churn** defeats cross-wake once-per-threshold for teammates (one re-block + forced turn per wake while parked over-threshold). **Decided 2026-06-06: accept-as-residual (1.5.4)** — under the sensor reframe (the hook reports crossings; the agent's operating instructions prescribe the response) the per-wake repeat is one fresh warning per wake while over-threshold, bounded and figure-accurate; cross-wake identity engineering rejected as over-engineering. The 1.5.4 release also de-engineered every checkpoint message to neutral sensor language (threshold policy lives in the or-* manuals, F10). **No open follow-ups remain.**
