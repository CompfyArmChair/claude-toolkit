...
6. **(1.5.2 teammate scoping)** After the 1.5.2 release: `claude plugin install claude-toolkit` (restart so the updated hook loads), run **Spike 9**, and record its RESULT inline. Spike 8 facet (3)'s scoping defect is not closed until Spike 9 is GREEN. — **DONE 2026-06-06 against installed 1.5.3 (`ed640b6`): facets (1)–(4) GREEN — Spike 8 facet (3)'s scoping defect is CLOSED. Facet (5) split: guard GREEN / cross-wake once-per-threshold RED — NEW follow-up (re-block at every wake while a teammate stays over-threshold; per-wake agent-identity churn). See Spike 9 RESULT.**
