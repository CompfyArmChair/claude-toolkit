...
| **Worker** | `or-implementer-task<N>` / `or-spec-reviewer-task<N>` / `or-code-quality-reviewer-task<N>` / `or-final-reviewer` | One task phase | No | No | Fresh per task; reviewers reaped on verdict, implementer kept through fix loops and reaped at gate-close |
...
- **Phase 0 — Preflight** (background teammate): detect mode (`idea` / `spec` / `plan`) from the input, ask the user — **directly, in its own pane** (plain text) — for a worktree name + base branch, create the worktree, and report `PREFLIGHT_OK` to the manager with the resolved mode, paths, and conventions. (A teammate, not a one-shot subagent — so it has its own user channel; the user talks to it directly, the manager stays idle, and setup questions are never relayed through the manager. `AskUserQuestion` is main-loop-only. Spike 4.)
