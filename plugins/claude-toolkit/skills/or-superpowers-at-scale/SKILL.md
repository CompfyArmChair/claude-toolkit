...
- **Phase 0 — Preflight** (background teammate): detect mode (`idea` / `spec` / `plan`) from the input, ask the user (plain text) for a worktree name + base branch, create the worktree, and report `PREFLIGHT_OK` to the manager with the resolved mode, paths, and conventions. (A teammate, not a one-shot subagent — so it has a user channel; `AskUserQuestion` is main-loop-only. Spike 4.)
